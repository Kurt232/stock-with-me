...
def print_socket(string, end='\n'):
    print(string, end=end)


def input_socket(string):
    print_socket(string, end='')
    var = input()
    return var
...
def assessment():
    ...
    for question_index in range(len(questions)):
        question = ''
        question += str(questions[question_index])
        question += '\n'
        # print_socket(questions[question_index])
        select = selects[question_index]
        for each in select:
            question += str(each)
            question += '\n'
            # print_socket(each)
        print_socket(question)
        answer = str(input_socket(''))
        while not (answer >= 'a' and answer <= 'e'):
            print_socket('Error! Please input again!')
            answer = str(input_socket(''))
        if answer == 'a':
            a += 1
        elif answer == 'b':
...
class Chat(object):
    ...
    def r_respond(self, session, text):
        text = self.__normalize(text)
        try:
            previous_text = self.__normalize(session.conversation.get_bot_message(-1))
        except IndexError:
    ...
    def _say(self, session, message):
        ...
        return response
    def r_say(self, session, message):
        session.conversation.append_user_message(message)
        response = self.r_respond(session, message.rstrip("!."))
        session.conversation.append_bot_message(response)
        return response
    ...
    def terminal_chat(callback, first_question, terminate):
        ...
        print_socket(str(first_question))
        input_sentence = ""
        while input_sentence != terminate:
            input_sentence = terminate
            input_sentence = input("")
            if input_sentence == 'assessment':
                assessment()
            elif input_sentence:
                print_socket(callback(input_sentence))

    # Hold a conversation with a chat bot
    def converse(self, first_question=None, terminate="quit", gui=False, session_id="general"):
        ...
        # self.terminal_chat(partial(self._say, session), first_question, terminate)


def demo(first_question=None, language="en", gui=False, **kwargs):
